...
macro_rules! style_for_close_open {
    () => {
        style!(
            /* Position at top right inside the sidebar */
            position: absolute;
            top: 10px;
            right: 10px;
...
pub fn SidebarContainer() -> impl IntoView {
    ...
    let close_open = style_for_close_open!();
    ...
    view! {
        <div class=get_class!(sidestyle)>
            <button
                on:click=move |_| {
                    is_sidebar_active.update(|active| *active = !*active);
                }
                class=get_class!(close_open)>
                { move || {
